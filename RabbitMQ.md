...
中间件（Middleware）是出于操作系统和应用系统之间的软件，也有人认为它应该属于操作系统中的一部分。人们在使用中间件时，往往时一组中间件集成在一起，构成一个平台（包括开发平台和运行平台），但在这组中间件中必须要有一个通信中间件，即中间件 = 平台 + 通信，这个定义也限定了只有用于分布式系统中才能称为中间件。同时还可以把它与支撑软件和实用软件区分开来。
...
6. Transaction事务（全局/局部）全局事务（分布式事务）局部事务（在同一数据库连接内的事务）
...
具体地说，中间件屏蔽了底层操作系统的复杂性，使开发人员面对一个简单而统一的开发环境，减少程序设计的复杂性，将注意力集中在自己的业务上，不必再为程序在不同系统软件上的移植而重复工作，从而大大减少了技术上的负担。中间件带给应用系统的，不只是开发的简便、开发周期的缩短，也减少了系统的维护、运行和管理的工作量，还减少了计算机总体费用的投入。

> **中间件特点**

为解决分布异构问题，人们提出了中间件（middleware）的概念。中间件使位于平台（硬件和操作系统）和应用之间的通用服务，如下图所示，这些服务具有标准的程序接口和协议。针对不同的操作系统和硬件平台，它们可以有符合接口和协议规范的多种实现。

![](http://zhaocan.fym233.cn/alebi9r5m)
...
**简单说：中间件有个很大的特点，是脱离于具体设计目标，而且备提供普遍独立功能需求的模块。这使得中间件一定是可替换的。如果一个系统设计中，中间件是不可替换的，不是架构、框架设计有问题，那么就是这个中间件，在别处可能是个中间件，在这个系统内是引擎。**
...
![](https://img-blog.csdnimg.cn/20210315211117172.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzQ0ODU4MjMw,size_16,color_FFFFFF,t_70)

![](https://img-blog.csdnimg.cn/20210315211123991.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzQ0ODU4MjMw,size_16,color_FFFFFF,t_70)
...
![](http://zhaocan.fym233.cn/u2ek6yp2f)
...
![](https://img-blog.csdnimg.cn/20210315211142836.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzQ0ODU4MjMw,size_16,color_FFFFFF,t_70)
...
![](https://img-blog.csdnimg.cn/20210315211154786.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzQ0ODU4MjMw,size_16,color_FFFFFF,t_70)
...
![](https://img-blog.csdnimg.cn/20210315211203857.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzQ0ODU4MjMw,size_16,color_FFFFFF,t_70)
...
![](https://img-blog.csdnimg.cn/20210315211305521.png)

谁来生产消息，存储消息和消费消息呢？

![](https://img-blog.csdnimg.cn/20210315211313442.png)
...
![](https://img-blog.csdnimg.cn/20210315211321315.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzQ0ODU4MjMw,size_16,color_FFFFFF,t_70)
...
![](https://img-blog.csdnimg.cn/20210315211331489.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzQ0ODU4MjMw,size_16,color_FFFFFF,t_70)
...
![](https://img-blog.csdnimg.cn/20210315211338807.png)
...
![](https://img-blog.csdnimg.cn/20210315211348873.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzQ0ODU4MjMw,size_16,color_FFFFFF,t_70)

> **常见的持久化方式**

![](https://img-blog.csdnimg.cn/20210315211441149.png)
...
![](https://img-blog.csdnimg.cn/2021031521141081.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzQ0ODU4MjMw,size_16,color_FFFFFF,t_70)

比如我在 APP上下了一个订单，我们的系统和服务很多，我们如何得知这个消息被哪个系统或者哪些服务器或者系统进行消费，那这个时候就需要一个分发的策略。这就需要消费策略。或者称之为消费的方法论

> **场景分析二**

![](https://img-blog.csdnimg.cn/20210315211404112.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzQ0ODU4MjMw,size_16,color_FFFFFF,t_70)
...
![](https://img-blog.csdnimg.cn/20210315211449988.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzQ0ODU4MjMw,size_16,color_FFFFFF,t_70)
...
![](https://img-blog.csdnimg.cn/20210315211459468.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzQ0ODU4MjMw,size_16,color_FFFFFF,t_70)

解说：生产者将消费发送到 Master节点，所有的都连接这个消息队列共享这块数据区域，Master节点负责写入，一旦 Master挂掉，slave节点继续服务。从而形成高可用

> **集群模式2 - Master-slave主从同步部署方式**

![](https://img-blog.csdnimg.cn/20210315211506280.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzQ0ODU4MjMw,size_16,color_FFFFFF,t_70)
...
![](http://zhaocan.fym233.cn/nevhowqjr)
...
![](http://zhaocan.fym233.cn/efh4n7x5is)

#### 1.3、安装Erlang

> **查看系统版本号**

![](http://zhaocan.fym233.cn/lbb97qehqc)
...
![](http://zhaocan.fym233.cn/8mra8it4l)
...
![](http://zhaocan.fym233.cn/ph5e9eqfz)
...
![](http://zhaocan.fym233.cn/rfudc2pcm)
...
![](http://zhaocan.fym233.cn/y0u986rjr5)

#### 2.3、小结

![](http://zhaocan.fym233.cn/0laxp0y1p)

### 3、RabbitMQ之Docker安装

#### 3.1、Dokcer安装RabbitMQ

> **虚拟化容器技术 - Docker的安装**

![](http://zhaocan.fym233.cn/trr2ilyzzg)

> **docker的相关命令**

![](http://zhaocan.fym233.cn/8i6k3nz2y)

> **安装rabbitmq**

![](http://zhaocan.fym233.cn/4ntcvj9fa7)
...
![](http://zhaocan.fym233.cn/p7lqednf9)

> **启动**

![](http://zhaocan.fym233.cn/606brfoh8)

访问网页，访问成功！

### 4、RabbitMQ的角色分类

![](http://zhaocan.fym233.cn/ww2mt9sky)

![](http://zhaocan.fym233.cn/63wsihcgps)
...
![](http://zhaocan.fym233.cn/maartmwzb)
...
![](http://zhaocan.fym233.cn/bj4xa88wn)
...
![](http://zhaocan.fym233.cn/26q7cgckl)

#### 2.3、AMQP消费者流转过程

![](http://zhaocan.fym233.cn/ld4038swe)

### 3、RabbitMQ的核心组成部分

#### 3.1、RabbitMQ的核心组成部分

![](http://zhaocan.fym233.cn/omvddhhui8)

![](http://zhaocan.fym233.cn/zl5rjkmqn9)

#### 3.2、RabbitMQ整体架构是什么样子的？

![](http://zhaocan.fym233.cn/3amzb1lfyu)

#### 3.3、RabbitMQ的运行流程

![](http://zhaocan.fym233.cn/vrlbw1bhgn)

#### 3.4、RabbitMQ支持的消息模型

![](http://zhaocan.fym233.cn/ubtrzfvh0o)

![](http://zhaocan.fym233.cn/w0jzkfzn3y)
...
#### 4.1、RabbitMQ的模式之发布订阅模式

> **图解**

![](http://zhaocan.fym233.cn/vyekf68tzj)
...
![](http://zhaocan.fym233.cn/1c4laqkt6)
...
![](http://zhaocan.fym233.cn/ykdl3o4poj)
...
#### 7.1、Work模式轮询模式（Round-Robin）

> 图解

![](http://zhaocan.fym233.cn/tnece6uibu)
...
**finalCChannel.basicConsume(queueName, true, deliverCallback, callback);**
...
![](http://zhaocan.fym233.cn/5ukc7a5tp9)
...
![](http://zhaocan.fym233.cn/b5sl90o5b)
...
![](http://zhaocan.fym233.cn/5ax7gb665)
...
![](http://zhaocan.fym233.cn/6vtihjtnlc)
...
![](http://zhaocan.fym233.cn/cftn1kyz4)

### 3、内存磁盘的监控

#### 3.1、RabbitMQ内存警告

![](http://zhaocan.fym233.cn/tgumt4681)
...
![](http://zhaocan.fym233.cn/702usz0n2p)

> **配置文件方式 rabbitmq.conf**

![](http://zhaocan.fym233.cn/dr2jvon2xo)

#### 3.3、RabbitMQ的内存换页

![](http://zhaocan.fym233.cn/6ez7elxld)

#### 3.4、RabbitMQ的磁盘预警

![](http://zhaocan.fym233.cn/lvt0quq8pq)

### 4、集群

![](http://zhaocan.fym233.cn/1r3sseceb4)

#### 4.1、集群搭建

配置的前提是你的 rabbitmq可以运行起来，比如`ps aix|grep rebbitmq`你能看到相关进程，又比如运行`rabbitmqct status`你可以看到类似如下信息而不报错：

![](http://zhaocan.fym233.cn/rtspc0hyqn)

#### 4.2、单机多实例搭建

![](http://zhaocan.fym233.cn/wfo2bwmtpd)

> **启动第二个节点**

![](http://zhaocan.fym233.cn/yx5wodhkpr)
...
![](http://zhaocan.fym233.cn/z3y6dqx3d2)

> **rabbit-2操作作为从节点**

![](http://zhaocan.fym233.cn/cylv937tey)

> **验证集群状态**

![](http://zhaocan.fym233.cn/t4zqlbasco)
...
![](http://zhaocan.fym233.cn/j814tkldyk)

> **小结**

![](http://zhaocan.fym233.cn/x6tcmuwww)
...
![](http://zhaocan.fym233.cn/pa0siu1kjw)
...
![](http://zhaocan.fym233.cn/b5eb73dsv)
...
![](http://zhaocan.fym233.cn/hknhslco7x)
...
![](http://zhaocan.fym233.cn/jgh8fld2d)
...
![](http://zhaocan.fym233.cn/mwgtpoja0l)

![](http://zhaocan.fym233.cn/79wzh0caq3)

##### ①、系统与系统之间的分布式事问题

![](http://zhaocan.fym233.cn/055gfwu3ob)
...
![](http://zhaocan.fym233.cn/f0eohslfvo)

##### ④、基于MQ的分布式事务消息的可靠生产问题-定时重发

![](http://zhaocan.fym233.cn/qyzz531zd4)

如果这个时候MQ服务器出现了异常和故障，那么消息是无法获取到回执信息。怎么解决呢?

![](http://zhaocan.fym233.cn/s4xgv8ae6f)

##### ⑤、基于MQ的分布式事务消息的可靠消费

![](http://zhaocan.fym233.cn/3syy3j5s9o)

##### ⑥、基于MQ的分布式事务消息的消息重发

![](http://zhaocan.fym233.cn/9443zh79ql)
...
![](http://zhaocan.fym233.cn/mtdgmlt6za)
